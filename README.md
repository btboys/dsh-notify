...
DeepSeek Harness (DSH) 通知插件，支持多种通知渠道，在**对话完成、暂停、失败、向你提问、需要授权或确认、TODO 任务进度推进**时自动发送通知。
...
- 📋 **TODO 进度** - Agent 发布/更新任务列表（`todo_write`）时推送清单与进度；按会话去抖，仅进度变化时推送
...
TODO 进度推送是带状态图标的任务清单 + 完成进度（超过 10 项自动折叠）：
...
- 📱 **命令菜单** — 聊天输入框旁的菜单按钮（`setMyCommands` 注册）提供斜杠命令，按钮点选即切换，无需记命令：

  | 命令 | 作用 |
  |---|---|
  | `/sessions` | 内联按钮列出最近对话（标题 + 工作区，过滤空会话和子代理），点选切换续接目标 |
  | `/workspace` | 内联按钮列出工作区，点选后沿用该工作区最新对话，没有则自动新建一个 |
  | `/current` | 查看当前续接的对话 |
  | `/help` | 命令帮助 |

  微信端同样可用：直接发送上述命令文字，菜单以编号列表推送，回复 `/sel s 序号` / `/sel w 序号` 完成选择
